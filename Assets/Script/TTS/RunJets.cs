...
public class RunJets : MonoBehaviour
{
	...
	public void DoInference(string ptext)
	{
		...
		Debug.Log($"Samples = {samples.Length} samples");
		//Debug.Log($"Samples per token = {samples.Length / (double)tokens.Length} spt");
		//Debug.Log($"Audio duration = {samples.Length / (double)samplerate} seconds");
